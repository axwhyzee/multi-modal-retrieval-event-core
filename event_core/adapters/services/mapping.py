...
def _namespace_from_obj_types(
    from_type: ObjectType, to_type: ObjectType
) -> str:
...
class AbstractMapper(ABC):
    @abstractmethod
    def set(
        self, from_type: ObjectType, to_type: ObjectType, key: str, val: ValueT
    ) -> None:
        raise NotImplementedError

    @abstractmethod
    def get(
        self, from_type: ObjectType, to_type: ObjectType, key: str
    ) -> ValueT:
        raise NotImplementedError
...
class FakeMapper(AbstractMapper):
    def __init__(self):
        self._namespaces: Dict[str, Dict[str, ValueT]] = defaultdict(dict)

    def set(
        self, from_type: ObjectType, to_type: ObjectType, key: str, val: ValueT
    ) -> None:
        namespace = _namespace_from_obj_types(from_type, to_type)
        self._namespaces[namespace][key] = val
    ...
    def delete(
        ...
        namespace = _namespace_from_obj_types(from_type, to_type)
        self._namespaces[namespace].pop(key)
